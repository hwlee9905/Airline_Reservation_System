...
6. 실행 시 오류 및 충돌 개선
## 기능 구현
- Seat Reservation DB System에 승객이 접속 하면 첫 화면에 기본적으로 출발지, 도착지, 탑승일, 인원 수, 좌석 등급, 예약 조회, 항공편 조회를 할 수 있는 화면이 나오며 오른쪽 상단에서 회원 가입 및 로그인을 할 수 있다.

- 승객이 Seat Reservation DB System에 승객/관리자, ID, 패스워드, 이름, 전화번호, 주소를 입력하여 회원가입을 할 수 있고, 회원가입이 완료 되면 승객은 Seat Reservation DB System에 로그인하여 항공편을 검색 할 수 있다. 

- 승객이 출발지, 도착지, 탑승일, 인원 수, 좌석 등급을 정하고 나서 비행 예약 버튼을 누르면 해당 조건에 맞는 항공편이 출발시간 -> 도착시간 – 가격순으로 뜨게 되고 원하는 항공편을 선택하면 예약할 수 있다.

- 승객은 예약이 완료된 후 예약을 확인하거나 당일을 제외한 날에 예약을 취소할 수 있다.

- 관리자는 Seat Reservation DB System에 승객/관리자, ID, 패스워드, 이름, 전화번호, 주소, 사원번호를 입력하여 회원가입을 할 수 있고, 회원가입이 완료 되면 관리자는 항공편 조회, 항공기 조회, 승객 정보 조회, 예약 정보 조회를 할 수 있다.

- 관리자는 항공편을 편입하거나 삭제할 수 있고, 특정 항공편의 항공기를 변경할 수 있으며, 전체 승객의 예약 정보를 확인할 수 있다. 

- 또한 비행이 여의치 않을 경우 해당 항공편을 삭제하고 해당 항공편에 관한 예약 정보를 환불 처리 할 수 있다.
## File Information

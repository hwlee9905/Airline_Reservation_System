...
<%	
 	...
 	int rows =Integer.parseInt(String.valueOf(session.getAttribute("rows2")));
 	...
 	int people=0;
	   ...
	   for(int i=0; i<count; i++){
		   ...
		   int k=0;
	        ...
	        if(a==0){
	        	out.print("<script> alert('환불에 실패했습니다');window.location.href = 'http://localhost:8080/DBairport/refund_p/listRS_reservatoin2.jsp';</script>");
	        }else {
	        	int b=DB.leftseat_up(flightnum,aircraftnum, people);
	        	out.print("<script> alert('환불했습니다');window.location.href = 'http://localhost:8080/DBairport/refund_p/listRS_reservatoin2.jsp';</script>");
	        }
